...
/* ---------------------------------------------------------------- */
/* Paw icon (brand mark, not lucide)                                  */
/* ---------------------------------------------------------------- */
...
const FAQ_ITEMS: FAQItem[] = [
  {
    ...
    a: "Pettr turns every vet visit, symptom, and daily note into one running health picture, instead of loose files in a drawer. We surface patterns your vet doesn't have time to piece together between appointments.",
  },
  {
    q: 'I already use a journal and go to the vet, why use Pettr?',
    a: "A paper journal remembers what you write down. Pettr remembers everything, cross-references it against your pet's history, and flags what actually matters before it becomes urgent, so your notes and your vet's records finally talk to each other.",
  },
  {
    q: 'My pet is in good health, can Pettr do anything for me?',
    a: 'Good health is easiest to protect when you have a baseline. Pettr tracks weight, habits, and routine care over time, so if anything drifts, you catch the change early instead of after it becomes a problem.',
  },
  {
    q: 'How does Pettr work with my vet?',
    a: 'Export a clean, dated summary before any appointment. Vets get context in seconds instead of a rushed recap, and anything discussed in the room can be logged straight back into your timeline.',
  },
  {
    q: "Is my pet's data private?",
    a: "Yes. Your pet's records belong to you. Nothing is sold, shared, or used to train anything without your say-so, and you can export or delete your data whenever you want.",
  },
]
  ...
  return (
    ...
    >
      <button
        ...
        className={
          'group flex w-full items-center gap-4 rounded-2xl border px-5 py-4 text-left transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#602037]/40 sm:px-6 sm:py-5 ' +
          (isOpen
            ? 'border-[#602037]/15 bg-white shadow-[0_10px_30px_-14px_rgba(96,32,55,0.35)]'
            : 'border-black/[0.06] bg-[#F1EFEA] hover:bg-[#EAE7E0]')
        }
      >
        <span
          className={
            'flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-transform duration-500 ' +
            (isOpen ? 'rotate-[8deg] bg-[#E3A73E]/20' : 'bg-[#E3A73E]/10 group-hover:rotate-[8deg]')
          }
        >
          <PawIcon className="h-[15px] w-[15px] text-[#E3A73E]" />
        </span>

        <span className="flex-1 text-[17px] font-semibold leading-snug text-[#14100F] sm:text-[19px]">
          {item.q}
        </span>

        <span
          className={
            'flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-black/10 text-[#14100F] transition-transform duration-300 ' +
            (isOpen ? 'rotate-45' : '')
          }
        >
      ...
      >
        <div className="min-h-0">
          <p className="px-5 pb-5 pt-3 text-[15px] leading-relaxed text-[#5b5854] sm:px-6 sm:pl-[4.25rem]">
            {item.a}
          </p>
        </div>
      </div>
...
export function LandingFAQ() {
  ...
  return (
    <Section>
      <SectionInner>
        {/* Same shadow-box pattern as the rest of the landing system, but
            this variant sits on a white surface: bg flips from bg-primary
            to white and text flips from white to black, everything else
            (radius, shadow, padding rhythm) stays identical. */}
        <div
          className="relative flex min-h-[65vh] flex-col justify-center overflow-hidden rounded-[30px] bg-white p-[50px] text-black lg:p-[70px]"
          style={shadowBoxStyle}
        >
          <div className="relative z-10 mx-auto grid w-full max-w-[1200px] gap-12 lg:grid-cols-[1.05fr_1fr] lg:gap-16">
            <Stack gap="sm">
              <LandingHeading as="h1">We're here to health.</LandingHeading>
              <LandingSubheading>& to answer all your questions</LandingSubheading>
            </Stack>
