...
export class FoodService {
  ...
  static calculateDryFoodRemaining(entry: DryFoodEntry): { remainingDays: number; depletionDate: Date; remainingWeight: number } {
    ...
    const dailyAmount = parseFloat(entry.dailyAmount);
    const bagWeight = parseFloat(entry.bagWeight);
  ...
  static calculateWetFoodRemaining(entry: WetFoodEntry): { remainingDays: number; depletionDate: Date; remainingWeight: number } {
    ...
    const daysSincePurchase = Math.floor((today.getTime() - purchaseDate.getTime()) / (1000 * 60 * 60 * 24));
    const dailyAmount = parseFloat(entry.dailyAmount);
    const totalWeight = entry.numberOfUnits * parseFloat(entry.weightPerUnit);
    
    const foodConsumed = Math.max(0, daysSincePurchase * dailyAmount);
    const remainingWeight = Math.max(0, totalWeight - foodConsumed);
    const remainingDays = dailyAmount > 0 ? Math.floor(remainingWeight / dailyAmount) : 0;
